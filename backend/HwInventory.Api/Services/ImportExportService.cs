...
public class ImportExportService(AppDbContext db)
{
    ...
    public async Task<int> ImportJsonAsync(Stream body, CancellationToken ct)
    {
        ...
        if (root.TryGetProperty("activities", out var acts))
        {
            foreach (var a in acts.EnumerateArray())
            {
                ...
                db.Activities.Add(new Activity
                {
                    HardwareId = hid,
                    ProjectId = pid,
                    Kind = Enum.Parse<ActivityKind>(a.GetProperty("kind").GetString() ?? "Used", true),
        ...
        if (root.TryGetProperty("hardware_configs", out var cfgs))
        {
            foreach (var c in cfgs.EnumerateArray())
            {
                ...
                {
                    HardwareId = hid,
                    Kind = Enum.Parse<HardwareConfigKind>(c.GetProperty("kind").GetString() ?? "Config", true),
    ...
    private static Hardware ReadHardware(JsonElement h, IdMap maps)
    {
        var entity = new Hardware
        {
            ...
            Status = h.TryGetProperty("status", out var st) ? Enum.Parse<HardwareStatus>(st.GetString() ?? "Available", true) : HardwareStatus.Available,
            Condition = h.TryGetProperty("condition", out var cd) ? Enum.Parse<HardwareCondition>(cd.GetString() ?? "Unknown", true) : HardwareCondition.Unknown,
    ...
    private static Project ReadProject(JsonElement p)
    {
        return new Project
        {
            ...
            Status = p.TryGetProperty("status", out var s) ? Enum.Parse<ProjectStatus>(s.GetString() ?? "Idea", true) : ProjectStatus.Idea,
            Priority = p.TryGetProperty("priority", out var pr) ? Enum.Parse<ProjectPriority>(pr.GetString() ?? "Medium", true) : ProjectPriority.Medium,
